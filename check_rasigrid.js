...
async function main() { 
  ...
  const res = await client.query('SELECT "id", "rasiGrid" FROM "Profile" WHERE "rasiGrid" IS NOT NULL'); 
  const formats = new Set();
  const examples = {};
  for (const row of res.rows) {
    const rasi = row.rasiGrid;
    if (!rasi) continue;
    let format = 'unknown';
  ...
  console.log("Formats found for rasiGrid:", Array.from(formats));
